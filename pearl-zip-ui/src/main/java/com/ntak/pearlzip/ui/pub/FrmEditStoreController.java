...
public class FrmEditStoreController {

    private static final Logger LOGGER = LoggerContext.getContext().getLogger(FrmEditStoreController.class);
    ...
    public void initData(TableView tblStore, Stage stage) {
        StoreRepoDetails repo = (StoreRepoDetails) tblStore.getSelectionModel().getSelectedItem();
        txtBoxName.setText(repo.name());
        txtBoxURL.setText(repo.url());
        txtBoxUsername.setText(repo.username());
        txtBoxPassword.setText(repo.password());
        ...
        btnEdit.setOnAction((e)-> {
           // Create Repository Entry Object and add to Map
            ...
            if (Objects.isNull(storeRepoDetails)) {
                return;
            }
           ...
           try (Connection connection = DriverManager.getConnection(storeRepoDetails.url(), storeRepoDetails.username(), storeRepoDetails.password())) {
               ...
               raiseAlert(Alert.AlertType.ERROR, resolveTextKey(TITLE_ISSUE_CONNECTING_DB), null, resolveTextKey(BODY_ISSUE_CONNECTING_DB), btnEdit.getScene().getWindow());
               return;
           } finally {
